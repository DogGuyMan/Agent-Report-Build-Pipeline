...
await build({
  ...
  alias: {
    ...
    "report-builder/svg": join(ROOT, "viz/svg.mjs"),
  },
  outfile: tmp,
  absWorkingDir: cwd,
  logLevel: "warning",
...
// 용어 자동 참조 — terms 가 있을 때만. 본문 글자의 모든 등장을 TermRef 마크업으로 감싼다(viz/wrap-terms.mjs).
...
// 경로 링크 — 본문의 경로 꼴 낱말을 실제 로컬 파일 · 폴더의 file:// 로. 없는 파일은 그대로 둔다(viz/link-paths.mjs).
{
  let repoRoot = cwd;
  try {
...
const css = readFileSync(join(ROOT, "viz/src/theme.css"), "utf8");
...
if (Array.isArray(data.terms) && data.terms.length > 0) {
  const r = await build({
    entryPoints: [join(ROOT, "viz/src/runtime/term-graph.ts")],
    bundle: true,
    minify: true,
    format: "iife",
    platform: "browser",
