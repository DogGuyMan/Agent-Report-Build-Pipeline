...
| 5 | B4 | 코드 | `check.mjs` |
| ~~6~~ | ~~B5~~ | ⛔ **취소 (2026-08-28)** | ~~기존 2건 재생성·대조~~ → 골든 조각 테스트로 대체 |
...
**사용자 결정(2026-08-28): 저작 실수로 보고 `tier`를 amber로 정정한다.** 다만 정본 파일 자체는 건드리지 않는다. `emoji` prop은 그대로 남으며 `test/golden.test.mjs`가 이 재현 능력을 고정한다.
...
>
> **원래 목적은 다른 수단으로 대체했다.** Phase 6 의 산출물은 두 보고서의 갱신본이 아니라
> "컴포넌트가 정본과 같은 마크업을 내는가" 라는 검증이었다. 이것은 `test/golden.test.mjs` 의
> **골든 조각 대조**로 옮겼다 — 정본에서 뜬 마크업 조각을 리터럴로 박고 `npm test` 가 상시 검사한다.
>
> **그 대체로 이미 드러난 사실 (2026-08-28 실측):**
> - 확신도 배지 6종 · 상태 배지 6종 · 판정 푸터 선택지 3종 — **정본과 바이트 동일**
> - 결정 표 헤더 3곳이 다르다 — 정본 `D#`/`Status`/`옵션 수` vs 컴포넌트 `#`/`상태`/`옵션`
> - 판정 푸터의 사유란과 안내문이 다르다 — 정본 `사유: &nbsp;` 와 `<strong>` 강조 vs 컴포넌트 평문
> - 위 불일치를 정본에 맞출지는 **미결정**이며, 골든 테스트가 현재 값을 고정해 변경을 감지한다
>
> **문서 전체의 바이트 동등은 원리적으로 불가능하다.** 정본은 들여쓰기와 줄바꿈을 갖는 반면
> `renderToStaticMarkup` 은 태그 사이에 공백을 내지 않는다. 대조가 성립하는 단위는
> **정본에서 한 줄로 나타나는 인라인 조각**뿐이다.
