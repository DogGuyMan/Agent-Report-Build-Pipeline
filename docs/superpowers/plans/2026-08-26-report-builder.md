...
| ~~2~~ | ~~B2~~ | ⛔ **기각 (2026-08-28)** | ~~`.dot` 2장 + 판정 시간 비교~~ |
| ~~3~~ | ~~A1~~ | ⛔ **취소 (2026-08-29)** | ~~소급 검증 보고서~~ — 재료 부재 + 옛 저장소 의존 |
| 4 | B3 | 코드 | `report` CLI + 컴포넌트 |
| 5 | B4 | 코드 | `check.mjs` |
| ~~6~~ | ~~B5~~ | ⛔ **취소 (2026-08-28)** | ~~기존 2건 재생성·대조~~ — 옛 출력을 기준 삼는 것 자체가 후퇴 |
...
# Phase 2 — B2 Decision-gate mode 파일럿  ⛔ 기각 (2026-08-28)

> **이 Phase 는 착수하지 않는다. 아래 체크박스를 따르지 말 것.**
>
> **기각 사유 (사용자 판단).** 착수 전 전제 조사에서 다음이 확인됐다.
>
> - **문제의식은 살아 있었다** — 결정 전 후보 비교가 표뿐이라는 상태는 정본 2건에서 재확인됐고,
>   `llm-load-reduction` 보고서의 D2 옵션표로 사례가 오히려 하나 늘었다.
> - **그러나 사례 선정의 근거 절반이 부식됐다** — 계획서 `:549` 는 "정답이 `Material` 로 확정된
>   사례라 검증이 명확하다"고 적었으나, 그 정답이 가리킨 코드가 대상 저장소에 없다.
>   `src/material.h` 부재, `ITechnique` 소스 히트 0건, git `-S` 검색에서도 소스 진입 이력 없음.
>   패스 목록의 현재 소유자는 `PassIterator`(`src/render/pass_iterator.h:67`)다.
> - **비교 대상 표가 없었다** — Task 2.3 이 요구하는 "같은 결정의 옵션표"가 어디에도 없어,
>   계획서에 없는 추가 작업(표 신규 작성)이 선행돼야 했다.
>
> **되살릴 조건:** 후보 비교를 그림으로 하려는 요구가 다시 생기고, **정답이 확정됐으면서
> 그 결정의 옵션표가 이미 존재하는** 사례가 나타날 때.
...
# Phase 3 — A1 소급 검증 (산출물은 문서, 코드 아님)  ⛔ 취소 (2026-08-29)

> **이 Phase 는 착수하지 않는다. 아래 체크박스를 따르지 말 것.**
>
> **취소 사유 두 겹.**
> 1. **재료가 없다** — 소급 검증은 "결정"과 "그 뒤의 결과(번복/존속)"를 짝지어야 성립하는데,
>    report-builder 의 유일한 보고서(`llm-load-reduction`)는 결정 6건이 전부 `[제안됨]`이고
>    그 계획의 Task 1~8 중 실행된 것이 0개다. 채점표의 세로축이 비어 있다.
> 2. **재료를 구하는 방향이 후퇴다** — 이 Phase 는 외부 Graphics 저장소의 설계 문서 60개를 표본으로
>    지목한다. 새 도구를 옛 프로젝트 이력에 다시 묶는 것이고, 사용자가 이를 "후퇴 증거"로 판정했다.
>
> **참고 — 취소 전에 실제로 수집된 표본이 있었다.** 외부 저장소에서 구조 신설 결정 16건을 뽑았고
> (존속 5 · 번복 11, 그중 코드 미진입 5), 검색어가 `신규` 를 놓쳐 최소 3건이 더 빠졌다는 것까지
> 확인했다. 그 표본은 폐기했다.
>
> **되살릴 조건:** report-builder 로 쌓은 결정들이 실제로 번복되거나 살아남은 이력이 충분히 생겼을 때.
...
**(2) 정본에 tier와 이모지가 어긋난 사례가 1건 있다.** (2026-08-28 정정 — 이 계획서는 원래 "2건"이라고 적었으나 전수 검사 실측은 1건이다.)

```html
<span class="conf-badge conf-green">🟡 80</span>   ← 🟡인데 green
```

🟡(amber)인데 클래스가 `conf-green`이다. **`emoji` prop으로 재정의를 허용해 두 경우를 모두 표현할 수 있게 한다.**

🔵 **2026-08-28 실측 — 정본 2개 파일의 `conf-badge` 24개를 전수 검사한 결과 불일치는 1건뿐이다.** 유일한 사례는 `2026-07-27-geometry-winding-ownership-design-review.html` 143번째 줄이고, `matrix-rain-parameterization` 쪽은 0건이다.

**사용자 결정(2026-08-28): 저작 실수로 보고 `tier`를 amber로 정정한다.** 다만 정본 파일 자체는 건드리지 않는다. `emoji` prop은 그대로 남는다 — 옛 출력을 재현할 일이 생길 때를 위한 여지다.
...
export interface Conf {
  ...
  /** tier 가 함의하는 이모지를 덮어쓴다. 정본의 tier/이모지 불일치 1건 재현용. */
  emoji?: string;
}
...
# Phase 6 — B5 기존 보고서 재생성과 대조  ⛔ 취소 (2026-08-28)

> **이 Phase 는 착수하지 않는다. 아래 체크박스를 따르지 말 것.**
>
> **취소 사유 (사용자 판단):** 대상이던 두 spec(`geometry-winding-ownership`,
> `matrix-rain-parameterization`)은 **이미 종료된 작업**이다. 그 보고서들을 다시 만들 실익이 없다.
>
> **목적 자체를 폐기했다.** Phase 6 이 답하려던 질문은 "컴포넌트가 옛 출력과 같은 마크업을 내는가"
> 였는데, **그 질문이 틀렸다.** 옛 출력은 열등판이고 새 컴포넌트가 그것을 따라갈 이유가 없다.
> 2026-08-28 에 이 대조를 테스트로 만들었다가(`test/golden.test.mjs`) 같은 날 폐기했다.
>
> **폐기 전에 관측된 차이는 기록해 둔다 (2026-08-28 실측):**
> - 확신도 배지 6종 · 상태 배지 6종 · 판정 푸터 선택지 3종 — 옛 출력과 바이트 동일
> - 결정 표 헤더 3곳이 다르다 — 옛 `D#`/`Status`/`옵션 수` vs 현재 `#`/`상태`/`옵션`
> - 판정 푸터가 다르다 — 옛 `사유: &nbsp;` 와 `<strong>` 강조 vs 현재 평문
> - **현재 컴포넌트가 기준이다.** 옛 것은 헤더에 영어가 섞이고 `&nbsp;` 이스케이프 잔재가 있다

**원래 완료 조건(인계 문서 §B-6):** 기존 보고서 2건을 새 파이프라인으로 재생성해 원본과 비교. **시각적으로 동등하면 성공.**
...
- [x] **Step 3: tier/이모지 불일치를 어떻게 처리할지 사용자에게 묻는다** — 2026-08-28 완료. 실측 1건, 결정은 정정(amber)
...
| ~~2~~ | ~~2.3~~ | ⛔ | ~~표 vs 그림 판정 시간 비교~~ — 2026-08-28 기각 |
| ~~3~~ | ~~3.5~~ | ⛔ | ~~D축 게이트~~ — 2026-08-29 취소. Phase 7 은 평가 없이 보류 |
| ~~3~~ | ~~—~~ | ⛔ | ~~D 점수 구간 경계 조정~~ — A1 취소로 판단 근거 소멸 |
| 5 | 5.1 | ❓ | "콘솔 에러 0건" 검사를 지금 넣지 않는 것에 동의하는지 |
| ~~6~~ | ~~6.2~~ | ✅ | ~~정본의 tier/이모지 불일치~~ — 2026-08-28 해소. 실측 1건, 결정은 정정(amber) |
