...
> 🔵 **정정 (2026-08-29 14:40, 첫 시험 뒤).** 사용자가 문항 수를 **5 → 3** 으로, 구간을 **맞힌 수 2~3 확실 / 0~1 모름, "모른다" 2회 이상 모름** 으로 바꿨다(애매 없음). 사유 — 용어 20개 × 5문항 = 100문항 첫 시험에서 피로 실측. 아래 코드의 상수 5 와 80%/40% 경계는 옛 규칙이다. **구현(`scripts/term/quiz.mjs`)이 정본이다.**
