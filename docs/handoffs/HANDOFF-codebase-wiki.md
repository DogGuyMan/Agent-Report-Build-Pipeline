...
> ### 🟢 재개하는 세션은 여기가 아니라 `RESUME-2026-08-28-track-c.md` 부터 읽어라
>
> 이 문서는 **Track C 의 정본**(결정 C-1~C-18 · Phase 기록 · 스키마 · 인용 검증 규정)이지
> 재개 진입점이 아니다. 현재 상태·미커밋 항목·컨벤션·바로 다음 한 걸음은
> **`docs/handoffs/RESUME-2026-08-28-track-c.md`** 에 있다.
>
> ⚠ 그 문서가 알려주는 함정 둘을 모르면 헛돈다 — 대상 저장소의 `out/codegraph-raw/` 는
> **gitignore 라 다른 머신에 없고**, 커밋 메시지에 **트레일러를 붙이면 안 된다.**
...
### Phase 10 — C# 위키 전량 생성 (서브에이전트 병렬) 🔵 2026-08-28

C-17(생략 규칙)이 확정돼 큰 모듈이 서술 가능해졌으므로 남은 9장을 **`deep-wiki:wiki-writer`
서브에이전트로 병렬 생성**했다. 모듈당 소스를 전량 정독해야 해서(§1 20·21·22 — LLM 에만 남는 셋)
오케스트레이터가 직접 쓰면 컨텍스트가 감당되지 않는다.

⚠ **1차 시도는 주간 한도(rate limit)로 6개 전부 실패 보고**를 냈으나, 확인해 보니
`controller.md`(493줄)·`uis.md`(376줄)는 **파일을 다 쓰고 보고 단계에서 끊긴 것**이었다.
**실패 알림을 곧이곧대로 믿지 말고 산출물을 확인할 것.**

**🔵 검증기가 실제 결함을 반복해서 잡았다 — 이 파이프라인의 값어치가 여기서 증명된다.**

| 결함 | 건수 | 정체 |
|---|---|---|
| **인용 경로 축약** | 6 | 같은 파일을 재인용할 때 `UI_StarRate.cs:50` 처럼 **파일명만** 남긴다. 사람이 읽기엔 자연스럽지만 링크로는 죽는다. **프롬프트로 경고해도 반복 발생**했다 |

→ `codegraph/fix_citation_paths.py` 로 기계 복구한다. 저장소에서 그 파일명이 **정확히 하나일 때만**
치환하고, 여러 개면 손대지 않고 보고한다(어느 것인지 기계가 정할 수 없다).
🔵 실측 — 6건 전부 모호 0 · 미발견 0 으로 복구 가능했다.

**검증기 자체의 오탐도 두 건 드러나 고쳤다** (파일럿이 도구를 개선시킨 두 번째 사례):

| 오탐 | 원인 | 수정 |
|---|---|---|
| 이름 대조 경고 24건 | 산문이 줄바꿈되면 인용은 이 줄, 심볼 이름은 **앞줄**에 있다 | 인접 줄까지 대조 |
| 〃 | `<!-- Sources: ... -->` 는 근거 **목록**이지 주장이 아닌데 이름이 없다고 경고 | 그 줄은 이름 대조 제외 |

🔵 오탐 24 → 4. **오염 시험으로 진탐지가 살아 있음을 확인**했고(L1 실패 1 · L2 실패 1 ·
이름 경고 1), 그 시험을 `test_normalize.py` 에 **영구 편입**했다(스크래치패드는 지워진다).

### Phase 10-1 — C# 위키 10장 완성 + C-18 집행 완료 🔵 2026-08-28

```
인용 745건 — L1 745/745 · L2 745/745 (실패 0)
L3  노드 437 · 간선 104 · 멤버·메서드 75 · 근거없음 129 · 이름경고 4
Mermaid — Graphviz 교체 1 · mmdc 렌더 32 · 실패 0 · 클라이언트 의존 0
```

10장 전량이 규격을 충족한다(frontmatter · At a Glance · Mermaid 2~5개 · Sources 주석 ·
미확인 영역 · Related Pages). 치환본은 `wiki-built/`(SVG 자산 33개), **원본은 검증기 대상으로 보존**한다.

**🔵 C-18(전면 치환)의 실질 이득이 여기서 드러났다 — mermaid 문법 오류 5건을 빌드 시점에 잡았다.**
클라이언트 렌더(`vitepress-plugin-mermaid`)에 맡겼다면 **브라우저에서 조용히 깨졌을** 것들이다.

| 원인 | 건수 | 처리 |
|---|---|---|
| `style X~T~ fill:...` — 제네릭에 style | 1 | 치환기 폴백(색 지시자만 제거, 그림 내용 불변) |
| `style 한글이름 fill:...` — 비ASCII 식별자에 style | 1 | 〃 |
| 라벨의 HTML 엔티티 `&lt;T&gt;` | 1 | 〃 — VitePress 호환용 이스케이프가 mmdc 에서는 역효과 |
| 라벨 안 백틱 `["` + 이름 + `"]` — markdown-string 문법 | 1 | 〃 |
| `-.라벨.->` 에 마침표가 든 라벨 | 1 | **원본 수정** — `-.->\|라벨\|` 이 올바른 표기 |

앞의 넷은 **일반 규칙이라 치환기가 자동 복구**하고, 마지막 하나만 개별 오류라 원본을 고쳤다.

### Phase 5-3 — 회귀 테스트 🔵 완료

`codegraph/test_normalize.py` — **28개 통과.** 정규화 계층의 "틀려도 오류가 안 나는" 자리를 고정한다:
kind 대응표(항등 금지) · R5 투과 목록(일반화 금지) · R7 정식 이름 · 모듈 경계 · 외부 노드 명명 ·
중첩 타입 `##` · 검증기 오염 시험 3종.
**골든은 합성이 아니라 실제 산출물**을 쓴다(C++ 191/417/20 · C# 231/540/10)。

🔵 **뮤테이션으로 테스트 자체를 검증했다** — kind 대응표를 항등으로 되돌리면
`test_clang_uml_kind_is_not_identity` 만, R5 를 일반화하면 `test_r5_cpp_is_list_based...` 만
실패한다. 과탐지도 미탐지도 없다.
