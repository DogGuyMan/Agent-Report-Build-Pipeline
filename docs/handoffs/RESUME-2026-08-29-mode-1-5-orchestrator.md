...
### 인계 대기 중인 작업 둘 (2026-08-29, 각각 다른 에이전트용)

두 문서 모두 **자기완결형**이다. 받는 에이전트는 그것 하나만 읽고 착수할 수 있다.

| 문서 | 무엇 | 왜 지금 |
|---|---|---|
| `HANDOFF-2026-08-29-cpp-clang-doc-vs-clang-uml.md` | **C++ 자유 함수 층을 채운다.** `clang-doc`(LLVM 번들, 이미 설치)이 함수 236개를 `파일:줄` 과 문서 주석까지 낸다 — `clang-uml` 은 0개다. 둘을 `normalize.py` 에서 합친다 | Mode 1 의 C++ 함수 층이 **0%** 였다. C# 은 roslyn-dump 가 이미 준다 |
| `HANDOFF-2026-08-29-warmup-incremental-cache.md` | **전수조사 증분 캐시.** graphify 의 `manifest.json`(mtime → 내용 해시 → 두 겹) 방식. **원안의 결함을 고친다** — `git ls-tree HEAD` 는 커밋본이라 더러운 트리를 "유효" 로 본다 | 재조사가 매번 전량이다. 🔵 저장소 하나에 26만~29만 토큰 |

**둘은 겹치지 않는다** — 앞은 `normalize.py`, 뒤는 새 `warmup.py` 다. 병렬로 돌려도 된다.
다만 앞이 `normalize.py` 를 만지므로, 뒤쪽 에이전트에게 **그 파일을 건드리지 말라고** 이미 적어 뒀다.
