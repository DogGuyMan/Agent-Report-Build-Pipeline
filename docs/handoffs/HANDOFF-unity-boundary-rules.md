...
**산출물** — `$CSHARP_REPO/codegraph-rules.toml` (340줄, `tomllib` 파싱 검증 통과).
단계 1~3 완료, 단계 4 는 지시대로 하지 않았다.

| 항목 | 결과 |
|---|---|
| `[[layer]]` | 6개 — **모듈 10개 전부 배치** (단계 1 정지 조건 통과) |
| `[[entrypoint]]` | **36개** — `unity_lifecycle` 5 + `unity_asset` 31 (단계 2 정지 조건 통과) |
| `[[allow]]` | **0개** (주석 예시만) — 무엇이 허용된 누수인지는 사용자만 안다 |

### 핸드오프 전제와 달랐던 것 둘

**1. 씬 컨트롤러가 4개가 아니라 5개다.** §3 단계 2 (가) 목록에 `Test/PlayGroundScene` 이 빠져 있었다.
그리고 `ResourceClearenceScene` 은 다른 넷과 달리 `BaseScene` 이 아니라 `MonoBehaviour` 를 직접 상속한다.

**2. §1-2 "모듈 간 의존은 미측정이다" 가 더는 사실이 아니다.** `roslyn-dump` 없이도
기존 probe(오류 0건 컴파일)로 폴더 단위 집계가 가능했다. **사용자 확정으로 `dependency` 간선까지 뽑았다.**

```
상속·실현·필드만        →  7 모듈 /   101건   (Fixture·Exceptions·@Editors 가 통째로 빠짐)
+ dependency            → 10 모듈 / 1,484건   (14배)
```

`Fixture/FoodCellFactory` 가 그 이유다 — 필드도 기반 클래스도 없는 `static partial class` 라
의존이 전부 메서드 본문에 있다. **구조만 보면 안 보이는 결합이 그 차이다.**

측정된 순환: **상호 의존 5개**(`Controller↔UIs` 163/13 · `Managers↔Utils` 117/2 ·
`UIs↔Managers` 12/33 · `Utils↔Interface` 15/1 · `Utils↔Data` 5/2), **3개 이상 순환 11개**.

### 에이전트가 하지 않은 판단 셋 (§0 금지 준수)

- **`[[layer]]` 이름과 순서** — `L0`~`L5` 로만 두고 "사용자 확정 필요" 를 명시했다. 위상 정렬 자체는
  계산이지만 **상호 의존 5쌍에서 "건수가 적은 방향" 을 무시한 것이 곧 설계 판단**이라, 무시한 방향 5개를 파일에 적었다.
- **`[[allow]]`** — 하나도 채우지 않았다.
- **애매한 폴더 4개** — 2026-08-27 사용자 지시("따로따로 분리. 설계 판단은 핸드오프 목적을 벗어난다")대로
  `Test`·`Fixture`·`Exceptions`·`@Editors` 를 각각 독립 모듈로 두고 합치지 않았다. §4 미결정 2·3번이 이렇게 해소됐다.

### §4 미결정 갱신

- ✅ **`codegraph-rules.toml` 위치** — 2026-08-27 사용자 확정: **대상 저장소 루트**. C++ 쪽도 같다.
- ✅ **`Test`·`Fixture` / `@Editors`** — 위 참조. 각각 독립 모듈.
- ⏳ **`UnityEvent` 배선을 진입점으로 볼 것인가** — 여전히 미결정. 별도 과제.

### 단계 3 — 위생 정리 (보고만, 지우지 않았다)

```
$DEV_ROOT/StickRushGame               2022.3.30f1  e681003  미커밋 1,001건  5.5G
$CSHARP_REPO 6000.0.71f1  bf54917  미커밋     0건  5.8G  ← 정본
```

### 단계 4 는 지금 가능해졌다

핸드오프는 `roslyn-dump` 이후로 미뤘으나 **모듈 의존이 측정됐으므로 순환 판정과 위반 검사가 지금 가능하다.**
막는 것은 도구가 아니라 **`[[layer]]` 확정**(사용자 몫)이다.
