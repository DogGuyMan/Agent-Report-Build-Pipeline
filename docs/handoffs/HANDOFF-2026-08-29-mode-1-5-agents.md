...
| # | 할 일 | 왜 아직인가 |
|---|---|---|
| ~~1~~ | ~~세 정의 파일 커밋~~ | **완료** `2aca41a` (2026-08-29 06:00) |
| ~~2~~ | ~~`mode-2-spec-report` 가 로드되는지~~ | **해소** — 정의 커밋 뒤 하네스가 셋 다 이름으로 부른다 |
| ~~3~~ | ~~실제 작업을 태워 보기~~ | **완료** — Mode 1: ⑤⑥(terms-db 우선 Task 1~7) · Mode 1.5: ⑦(3문항 규칙) · Mode 2: ⑧⑨⑩⑪⑫(아코디언 · 자동 참조 · 경로 링크 · 물리 · 슬라이더). 소유 밖 변경 0, 커밋 0, 하네스가 오케스트레이터 오류 3건 잡음 |
