...
> **이 문서가 Mode 1.5 구현의 단일 진입점이다.** 새 세션은 이것 하나만 읽고 이어갈 수 있어야 한다.
...
**어디까지 왔나** — Mode 1.5(용어 이해도 벤치마크)의 **계획서가 완성되고 채점 구간까지 사용자가 확정**했다.
구현은 0줄이다. 계획서 10개 Task 중 착수된 것이 없다.

**바로 다음 한 걸음** — 아래 §3 의 위상 정렬에 따라 **L0 의 Task 0.1 을 시작하라.** 그것 하나만 끝나면
L1 에서 **Task 0.2 · 1.1 · 2.1 세 개를 서브에이전트로 동시에** 띄울 수 있다.
...
| 채점 | 한 용어당 **5문항**. 맞힌 수 **4~5 확실 / 2~3 애매 / 0~1 모름**. "모른다" 3회 이상이면 모름 |
...
|---|---|---|---|---|
| **L0** | 0.1 | — | 직렬 | 없음 |
| **L1** | 0.2 · 1.1 · 2.1 | **3개 동시** | 서브에이전트 | 0.1 (0.2 만). 1.1·2.1 은 사실상 무의존 |
| **L2** | 0.3 · 3.1 | 2개 동시 | 서브에이전트 | 0.3 ← 0.2 / 3.1 ← 2.1 (**파일 충돌**) |
| **L3** | 4.1 | — | 서브에이전트 | 3.1 (**파일 충돌**) |
| **L4** | 5.1 | — | 서브에이전트 | 4.1 (이름 참조) |
| **L5** | 5.2 | — | 서브에이전트 | 5.1 |
| **L6** | 6.1 | — | **오케스트레이터 직접** | 0.2 · 2.1 · 3.1 · 4.1 전부 |
