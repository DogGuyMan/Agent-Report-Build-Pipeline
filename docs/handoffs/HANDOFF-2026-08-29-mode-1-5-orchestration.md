...
**어디까지 왔나 (2026-08-29 갱신)** — 계획서 10개 Task 중 **8개 완료.** L0~L3 전부 커밋됐고 5.1 이 진행 중이다.
**Mode 1.5 의 CLI 파이프라인 `collect → grade → emit` 이 end-to-end 로 완주**했다(스크래치패드 실측).

**바로 다음 한 걸음** — 5.1 이 끝나면 5.2, 그다음 6.1(스킬). **직렬 사슬이라 병렬로 못 쪼갠다.**
여러 세션을 열 거라면 아래 §0 의 슬롯 분배를 따르라 — 병렬의 가치는 이 계획서 **밖**에 있다.
